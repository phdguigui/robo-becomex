...
export default {
  ...
  methods: {
    contractArm () {
      ...
      this.$axios.post(`https://localhost:7237/api/elbow/right/contract?position=${positionValue}`)
        .then((res) => {
          if (res.data) {
    ...
    fistPositive () {
      ...
      this.$axios.post(`https://localhost:7237/api/fist/right/rotate?angle=${angleValue}`)
        .then((res) => {
          if (res.data) {
